import { Observable, ReplaySubject, Subscription, filter, map, of, switchMap, take, tap } from "rxjs";
...
export abstract class SkillItemRelatedComponentBase<TEditorViewModel extends { IsActive: boolean, Id?: number }, TInputModel> implements OnDestroy, OnInit {
	...
	protected refresh(): Observable<any> {
		...
		let pipe = this.state.SelectedNode$.pipe(
			take(1),
			switchMap(
		...
		return this.createViewModel(pipe)
			.pipe(
				tap(x => this.data = x)
			);
	}
