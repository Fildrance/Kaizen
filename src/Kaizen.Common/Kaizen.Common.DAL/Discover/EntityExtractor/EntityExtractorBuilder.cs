...
namespace Kaizen.Common.DAL.Discover.EntityExtractor
{
	...
	public class EntityExtractorBuilder<TSelector, TEntity> where TEntity : class
	{
		...
		/// <returns>Passed builder with added configuration.</returns>
		public EntityExtractorBuilder<TSelector, TEntity> AddDiscoverRule(Func<TSelector, bool> canUse, Func<TSelector, Task<TEntity>> tryLoad)
		{
		...
		private void EnsureHaveAnyDiscoverRule()
		{
			if (!_discoverRules.Any())
			{
